...
def create_usuario_blueprint():
    try:
        ...
        if email.split('@')[1] == "professor.unip.com" or email.split('@')[1] == "aluno.unip.com" or email.split('@')[1] == "admin.unip.com":
...
def sign_in_usuario():
    try:
        ...
        usuarios = get_all_usuario()
        usuario = next((user for user in usuarios if user['email'] == email and user['senha'] == senha_digitada_hash), None)
