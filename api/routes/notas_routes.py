...
notas_bp = Blueprint('usuario', __name__)
...
@notas_bp.route('/buscar/<int:usuario_id>', methods=['GET'])
...
@notas_bp.route('/deletar/<int:usuario_id>', methods=['DELETE'])
...
@notas_bp.route('/atualizar/<int:usuario_id>', methods=['PUT'])
...
@notas_bp.route('/signin', methods=['POST'])
def sign_in_usuario():
    try:
        data = request.get_json()
        email = data['email']
